...
const tokenManager = require("../structs/tokenManager.js");

client.once("ready", () => {
    log.bot("Bot is up and running!");
    
    // Add tokens to existing users when bot starts
    tokenManager.addTokensToExistingUsers(client);
        ...
        if (!config.bBackendStatusChannelId || config.bBackendStatusChannelId.trim() === "") {
            ...
            if (!channel) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription("Eclipse Backend is now online")
                    .setColor("Green")
                    .setThumbnail("https://imgur.com/WT5gnsA.png")
                    .setFooter({
                        text: "Eclipse Backend",
    ...
    if (config.discord.bEnableInGamePlayerCount) {
        function updateBotStatus() {
            if (global.Clients && Array.isArray(global.Clients)) {
                client.user.setActivity(`${global.Clients.length} player(s)`, { type: 3 }); // 3 = WATCHING
            }
        }
