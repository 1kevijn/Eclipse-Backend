...
app.post("/fortnite/api/game/v2/profile/*/client/ClientQuestLogin", verifyToken, async (req, res) => {
    ...
    // Check for free Battle Pass eligibility on login
    if (req.query.profileId == "athena") {
        await checkFreeBattlePassEligibility(req.user.accountId, profiles);
    }
...
app.post("/fortnite/api/game/v2/profile/*/client/:operation", verifyToken, async (req, res) => {
    ...
    switch (req.params.operation) {
        ...
        case "IncrementNamedCounterStat": 
            // Check if this is a match completion stat
            if (req.body.counterName && req.body.counterName.includes("br_matchesplayed")) {
                await handleMatchCompletion(req.user.accountId, profiles);
            }
            break;
...
// Function to check if user is eligible for free Battle Pass (alternative method)
async function checkFreeBattlePassEligibility(accountId, profiles) {
    try {
        const user = await User.findOne({ accountId: accountId });
        if (!user || user.freeBattlePassClaimed) {
            return;
        }

        const athena = profiles.profiles["athena"];
        
        // Check if Battle Pass is already purchased
        if (athena.stats.attributes.book_purchased) {
            return;
        }

        // Check if user has played at least one match (simple check)
        // This is a fallback method in case IncrementNamedCounterStat doesn't work
        const hasPlayedMatch = athena.stats.attributes.level > 1 || 
                              athena.stats.attributes.accountLevel > 1 ||
                              Object.keys(athena.items).length > 50; // Has some items indicating gameplay

        if (hasPlayedMatch) {
            await handleMatchCompletion(accountId, profiles);
        }
    } catch (error) {
        console.error("Error checking free Battle Pass eligibility:", error);
    }
}

// Function to handle match completion and give PREMIUM Battle Pass (OfferId: 77F31B7F83FB422195DA60CDE683671D)
async function handleMatchCompletion(accountId, profiles) {
    try {
        // Check if user already claimed Premium Battle Pass
        const user = await User.findOne({ accountId: accountId });
        if (!user || user.freeBattlePassClaimed) {
            return; // User already claimed or doesn't exist
        }
        ...
        // Check if Battle Pass is already purchased
        if (athena.stats.attributes.book_purchased) {
            return; // Battle Pass already purchased
        }

        // Get season info
        const memory = functions.GetVersionInfo({ headers: { "user-agent": "Fortnite/++Fortnite+Release-5.41-CL-4363240 Windows/10.0.17763.1.256.64bit" } });
        const OnlySeasonNumber = memory.season;

        // Load Battle Pass data for current season
        let BattlePass;
        try {
            const season = `Season${OnlySeasonNumber}`;
            BattlePass = JSON.parse(fs.readFileSync(path.join(__dirname, "../responses/Athena/BattlePass/", `${season}.json`), "utf8"));
        } catch (error) {
            console.error(`No Battle Pass data found for season ${OnlySeasonNumber}`);
            return;
        }

        // UNLOCK PREMIUM BATTLE PASS (OfferId: 77F31B7F83FB422195DA60CDE683671D)
        console.log(`🎯 UNLOCKING PREMIUM BATTLE PASS for user ${accountId}`);
        console.log(`📋 Battle Pass OfferId: ${BattlePass.battlePassOfferId}`);
        console.log(`🎮 DevName: BR.Season5.BattlePass.01`);
        
        // Use the EXACT same logic as the real purchase system (lines 1923-1947)
        var lootList = [];
        var EndingTier = athena.stats.attributes.book_level || 1;
        
        // ACTIVATE PREMIUM BATTLE PASS (exact same as real purchase)
        athena.stats.attributes.book_purchased = true;

        // Add Battle Pass token to common_core (EXACT same as real purchase)
        const tokenKey = `Token:Athena_S${OnlySeasonNumber}_NoBattleBundleOption_Token`;
        const tokenData = {
            "templateId": `Token:athena_s${OnlySeasonNumber}_nobattlebundleoption_token`,
            "attributes": {
        ...
        console.log(`🎫 Added Battle Pass token: ${tokenKey}`);

        // Process ALL tiers up to current level (EXACT same logic as real purchase)
        for (var i = 0; i < EndingTier; i++) {
            var FreeTier = BattlePass.freeRewards[i] || {};
            var PaidTier = BattlePass.paidRewards[i] || {};
            var ItemExists = false;
            
            console.log(`🎁 Processing Tier ${i + 1} rewards...`);
            
            // Process FREE tier rewards (same as real purchase)
            for (var item in FreeTier) {
                if (item.toLowerCase() == "token:athenaseasonxpboost") {
                    athena.stats.attributes.season_match_boost = (athena.stats.attributes.season_match_boost || 0) + FreeTier[item];
                    console.log(`⚡ Added ${FreeTier[item]} Season XP Boost`);
                }
                if (item.toLowerCase() == "token:athenaseasonfriendxpboost") {
                    athena.stats.attributes.season_friend_match_boost = (athena.stats.attributes.season_friend_match_boost || 0) + FreeTier[item];
                    console.log(`👥 Added ${FreeTier[item]} Friend XP Boost`);
                }
                if (item.toLowerCase().startsWith("currency:mtx")) {
                    for (var key in common_core.items) {
                        if (common_core.items[key].templateId.toLowerCase().startsWith("currency:mtx")) {
                            if (common_core.items[key].attributes.platform.toLowerCase() == "shared") {
                                common_core.items[key].quantity += FreeTier[item];
                                console.log(`💰 Added ${FreeTier[item]} V-Bucks`);
                                break;
                            }
                        }
                    }
                ...
                if (item.toLowerCase().startsWith("homebasebanner")) {
                    for (var key in common_core.items) {
                        if (common_core.items[key].templateId.toLowerCase() == item.toLowerCase()) {
                            common_core.items[key].attributes.item_seen = false;
                            ItemExists = true;
                            break;
                        }
                    ...
                    if (ItemExists == false) {
                        var ItemID = functions.MakeID();
                        var Item = { "templateId": item, "attributes": { "item_seen": false }, "quantity": 1 };
                        common_core.items[ItemID] = Item;
                        console.log(`🏴 Added Banner: ${item}`);
                    }
                    ItemExists = false;
                }
                if (item.toLowerCase().startsWith("athena")) {
                    ...
                    if (ItemExists == false) {
                        ...
                        const Item = { "templateId": item, "attributes": { "max_level_bonus": 0, "level": 1, "item_seen": false, "xp": 0, "variants": [], "favorite": false }, "quantity": FreeTier[item] };
                        athena.items[ItemID] = Item;
                        console.log(`🎮 Added FREE Item: ${item}`);
                    }
                    ItemExists = false;
                }
                lootList.push({
            ...
            // Process PREMIUM tier rewards (this is what makes it premium!)
            for (var item in PaidTier) {
                if (item.toLowerCase() == "token:athenaseasonxpboost") {
                    athena.stats.attributes.season_match_boost = (athena.stats.attributes.season_match_boost || 0) + PaidTier[item];
                    console.log(`⚡ Added ${PaidTier[item]} PREMIUM Season XP Boost`);
                }
                if (item.toLowerCase() == "token:athenaseasonfriendxpboost") {
                    athena.stats.attributes.season_friend_match_boost = (athena.stats.attributes.season_friend_match_boost || 0) + PaidTier[item];
                    console.log(`👥 Added ${PaidTier[item]} PREMIUM Friend XP Boost`);
                }
                if (item.toLowerCase().startsWith("currency:mtx")) {
                    for (var key in common_core.items) {
                        if (common_core.items[key].templateId.toLowerCase().startsWith("currency:mtx")) {
                            if (common_core.items[key].attributes.platform.toLowerCase() == "shared") {
                                common_core.items[key].quantity += PaidTier[item];
                                console.log(`💰 Added ${PaidTier[item]} PREMIUM V-Bucks`);
                                break;
                            }
                        }
                    }
                ...
                if (item.toLowerCase().startsWith("homebasebanner")) {
                    ...
                    if (ItemExists == false) {
                        ...
                        var Item = { "templateId": item, "attributes": { "item_seen": false }, "quantity": 1 };
                        common_core.items[ItemID] = Item;
                        console.log(`🏴 Added PREMIUM Banner: ${item}`);
                    }
                    ItemExists = false;
                }
                if (item.toLowerCase().startsWith("athena")) {
                    ...
                    if (ItemExists == false) {
                        ...
                        // Special logging for Season 8 Tier 1 Premium Skins
                        if (item === "AthenaCharacter:cid_161_athena_commando_m_drift") {
                            console.log(`🦜 Added PREMIUM SKIN: Drift (Tier 1)`);
                        } else if (item === "AthenaCharacter:cid_163_athena_commando_f_viking") {
                            console.log(`🐉 Added PREMIUM SKIN: Huntress (Tier 1)`);
                        } else {
                            console.log(`🎮 Added PREMIUM Item: ${item}`);
                        }
                    }
                    ItemExists = false;
                }
                if (item.toLowerCase().startsWith("challengebundleschedule:")) {
                    var ItemID = functions.MakeID();
                    const Item = { 
                        "templateId": item, 
                        "attributes": { 
                    ...
                    athena.items[ItemID] = Item;
                    console.log(`📋 Added PREMIUM Challenge Bundle: ${item}`);
                }
                lootList.push({
                    "itemType": item,
                    "itemGuid": item,
        ...
        // Create gift box notification with all premium rewards
        const giftBoxId = functions.MakeID();
        const giftBox = {
            "templateId": "GiftBox:gb_battlepass",
            "attributes": {
                "max_level_bonus": 0,
                "fromAccountId": "Eclipse-Server",
                "lootList": lootList,
                "params": {
                    "userMessage": `🎯 PREMIUM BATTLE PASS UNLOCKED! You received ${lootList.length} premium rewards up to tier ${EndingTier}! Enjoy the Pirate Progressive & Dragon Ninja skins!`
                },
                "giftedOn": new Date().toISOString()
            },
            "quantity": 1
        ...
        // Mark user as having claimed Premium Battle Pass
        await User.updateOne(
            { accountId: accountId },
            { $set: { freeBattlePassClaimed: true } }
        );

        console.log(`🎯 PREMIUM Battle Pass granted to user ${accountId} after match completion - Level ${EndingTier} with ${lootList.length} premium rewards`);
        console.log(`🦜 Premium Tier 1 Skins: Drift & Huntress unlocked!`);

    } catch (error) {
        console.error("Error granting Premium Battle Pass:", error);
    }
}
