...
app.get("/fortnite/api/game/v2/matchmakingservice/ticket/player/*", verifyToken, async (req, res) => {
    ...
    if (!req.user.matchmakingId) {
        log.debug("No matchmakingId found, creating new one");
        req.user.matchmakingId = functions.MakeID().replace(/-/ig, "");
    }
    ...
    log.debug(`Returning matchmaker ticket for user ${req.user.accountId}`);
    log.debug(`Service URL: ${serviceUrl}`);
    log.debug(`Matchmaking ID: ${req.user.matchmakingId}`);
