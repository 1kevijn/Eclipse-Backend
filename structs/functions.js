...
async function registerUser(discordId, username, email, plainPassword, token) {
    ...
    const emailFilter = /^([a-zA-Z0-9_\.\-])+\@(([a-zA-Z0-9\-])+\.)+([a-zA0-9]{2,4})+$/;
    ...
    try {
        await User.create({
            ...
            matchmakingId,
            token // Speichere den generierten Token in der Datenbank
        }).then(async (i) => {
